...
router.get('/stats', authenticateToken, async (req, res) => {
  try {
    const [totalRequests, todayRequests, avgLatency, successRate, providerStats] = await Promise.all([
      ...
      pool.query(`SELECT provider, COUNT(*) as count, AVG(latency) as avg_latency FROM requests WHERE api_key_id IN (SELECT id FROM api_keys WHERE user_id = $1) GROUP BY provider`, [req.user.id]),
    ]);
...
router.get('/history', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT id, provider, model, status_code, latency, prompt_tokens, completion_tokens, error_message, created_at FROM requests WHERE api_key_id IN (SELECT id FROM api_keys WHERE user_id = $1) ORDER BY created_at DESC LIMIT $2 OFFSET $3',
