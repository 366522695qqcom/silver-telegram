...
router.get('/realtime', authenticateToken, async (req, res) => {
  const cacheKey = `realtime_${req.user.id}`;
...
router.get('/stats', authenticateToken, async (req, res) => {
  ...
  try {
    const [overviewStats, providerStats, modelStats] = await Promise.all([
      query(
        `SELECT
      ...
      query(
        `SELECT provider, COUNT(*) as count, AVG(latency) as avg_latency, SUM(cost) as total_cost
        FROM requests WHERE api_key_id IN (SELECT id FROM api_keys WHERE user_id = ?)
        GROUP BY provider`,
        [req.user.id]
      ),
    ...
    const row = overviewStats.rows[0] || {};
    ...
    const data = {
      ...
      today_requests: Number(row.today_count) || 0,
...
router.get('/daily', authenticateToken, async (req, res) => {
  const cacheKey = `daily_${req.user.id}`;
  const cached = monitorCache.get(cacheKey);
  if (cached) {
    return res.json(cached);
  }
  ...
  try {
    ...
    monitorCache.set(cacheKey, days);
    res.json(days);
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
