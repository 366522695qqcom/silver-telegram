...
router.get('/', authenticateToken, async (req, res) => {
  try {
    const result = await query(
      'SELECT id, provider_name, provider_type, base_url, enabled, avg_latency, last_success_at, last_failed_at, created_at FROM providers WHERE user_id = ?',
      [req.user.id]
    );
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await query(
      'SELECT id, provider_name, provider_type, base_url, enabled, avg_latency, last_success_at, last_failed_at, created_at FROM providers WHERE id = ?',
      [id]
    );
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await query('SELECT id, provider_name, provider_type, base_url, enabled, avg_latency, last_success_at, last_failed_at, created_at FROM providers WHERE id = ?', [req.params.id]);
    res.json(result.rows[0]);
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
...
router.post('/:id/toggle', authenticateToken, async (req, res) => {
  try {
    ...
    const updated = await query('SELECT id, provider_name, provider_type, base_url, enabled, avg_latency, last_success_at, last_failed_at, created_at FROM providers WHERE id = ?', [req.params.id]);
